...
class NotificationService {
  ...
  async sendCampaignNotification(campaignData, targetCenters = []) {
    try {
      ...
      const response = await apiService.post('/notifications/campaign', notificationData);
  ...
  async sendUrgentRecallNotification(recallData) {
    try {
      ...
      const response = await apiService.post('/notifications/urgent', urgentNotificationData);
      ...
      if (response.success) {
        ...
        throw new Error(response.message || 'Failed to send urgent notification');
      }
    } catch (error) {
      return {
        success: false,
  ...
  async confirmNotificationReceived(notificationId, centerId, response) {
    try {
      ...
      const result = await apiService.post('/notifications/confirm', confirmationData);
      return result;
    } catch (error) {
      return { 
        success: false, 
